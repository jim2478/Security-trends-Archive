...
예시의 `stats` 는 실제 `daily/2026-05-13.json` 의 값입니다(목록과 알림은 줄임). 단, 그 파일에는
`cve_total` / `alerts_with_cve` 가 없습니다. 두 필드는 B-9 반영 후에 쓰인 파일에만 있고, 예시의
두 값은 지금 코드가 이 파일에 이어서 누적할 때 채우는 값입니다(`cve_total` 은 `cve_count` 17 을
이어받고, `alerts_with_cve` 는 `alerts[]` 중 `cve_ids` 가 있는 9건). [과거 데이터 주의](#과거-데이터-주의) 참고.
...
|------|------|------|
| ~ 2026-05-14 10:07 KST | 실제 실행 횟수 (시간별 파일에서 이관, 대부분 24) | 파일 있음 (예: `2026-04-19.json`) |
| 2026-05-14 10:07 KST ~ 0건 기록 복구(B-9) 반영 전 | **알림이 나간 실행 횟수만** 셈 | **파일 없음** |
| 0건 기록 복구(B-9) 반영 후 | 실제 실행 횟수 | 파일 있음 (`alerts: []`) |

- 경계는 날짜가 아니라 시각입니다. 2026-05-14 10:07 KST 에 시간별 파일 구조가 `daily/` 통합
  구조로 바뀌면서 알림 0건인 실행을 쓰지 않게 됐습니다. 그래서 `2026-05-14.json` 은 두 방식이
  섞인 전환일입니다. 그 전 실행은 알림 유무와 상관없이 모두 세어졌고, 그 뒤 실행은 알림이 나간
  경우만 세어졌습니다. 이 파일의 알림(01·03·05시)은 모두 전환 전이므로 `scan_runs` 11 은 전환 전
  실행분이고, 전환 뒤 실행 횟수는 알 수 없습니다.
